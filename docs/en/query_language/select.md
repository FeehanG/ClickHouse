...
There can be different types of columns that match the `COLUMNS` expression. If `COLUMNS` doesn't match any columns and is the only expression in `SELECT`, ClickHouse throws an exception.
